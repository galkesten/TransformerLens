...
class AbstractAttention(ABC, nn.Module):
    ...
    def forward(
        ...
        if self.cfg.qk_layernorm:
            # Rearrange q and k to concatenate the last two dimensions
            q_flattened = einops.rearrange(q, "batch pos head_index d_head -> batch pos (head_index d_head)")
            k_flattened = einops.rearrange(k, "batch kv_pos head_index d_head -> batch kv_pos (head_index d_head)")
            # Apply LayerNorm to the concatenated dimensions
            q_normalized = self.q_ln(q_flattened)
            k_normalized = self.k_ln(k_flattened)
            # Rearrange back to the original shape
            q = self.hook_ln_q(einops.rearrange(q_normalized, "batch pos (head_index d_head) -> batch pos head_index d_head", 
                            head_index=self.cfg.n_heads, d_head=self.cfg.d_head))
            k = self.hook_ln_k(einops.rearrange(k_normalized, "batch kv_pos (head_index d_head) -> batch kv_pos head_index d_head", 
                            head_index=self.cfg.n_heads, d_head=self.cfg.d_head))
